...
```{r}
summary(lm_step_wo <- step(lm1_wo <- lm(Salary ~ fTeam+fPosition+fCollege+Age+Height_i+Weight, data=ap_cl_f_wo)))
```

### Box Cox

Despite an estimated power of $-0.33$, the Box-Cox Transformation doesn't appear to contribute aything to the model, as it results in the same reduced model with a reduced adjusted $R^2$.
...
summary(lm_step_trans <- step(lm1_trans <- lm(bcPower(Salary, Y1)~fTeam + fPosition + fCollege + Age + Height_i + Weight, data = ap_cl_f)))
```
