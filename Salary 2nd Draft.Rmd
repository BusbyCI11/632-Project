...
summary(lm2 <- lm(Salary ~ fPosition+fCollege+poly(Age,3)+Height_i+Weight, data=ap_tibble))
```

```{r}
# Removal of `Height_i` due to no significance
summary(lm3 <- lm(Salary ~ fPosition+fCollege+poly(Age,3)+Weight, data=ap_tibble))
```

```{r}
# Removal of `fCollege`
summary(lm4 <- lm(Salary ~ fPosition+poly(Age,3)+Weight, data=ap_tibble))
```

```{r}
lm1 <- lm(Salary ~ fTeam+fPosition+fCollege+poly(Age,3)+Height_i+Weight, data=ap_tibble)
...
ap_cl_f_wo <- ap_tibble[-which(abs(rstandard(lm_step)) > 2 | hatvalues(lm_step) > .1),]
```
...
### Linear Regression pr - Need to Change Variable Names
...
summary(lm1_log <- lm(log(Salary) ~ fTeam+fPosition+fCollege+Age+Height_i+Weight, data=ap_tibble))
...
data_train <- glm(fSalary.Dummy ~ nTeam+nPosition+nCollege+Age+Height_i+Weight,family=binomial, subset = train, data=ap_tibble)
...
summary(glmFit <- train(fSalary.Dummy ~ nTeam+nPosition+nCollege+Age+Height_i+Weight, data=ap_tibble, method="glmStepAIC"))
...
#sqrt can not be used for factors
#plot(predict(lm4), sqrt(ap_tibble$fPosition),
    # xlab = "Fitted Values", ylab = "Age")


library(corrplot)
nba.matrix <- as.matrix(ap_tibble)
corrplot(cor(nba.matrix), is.corr = FALSE, method = "circle", type = "upper")

corrplot(cor(ap_tibble %>% 
               select(fPosition, 
                      Age, Weight, contains("%")), 
             use = "complete.obs"), 
...
stats_salary_cor <- 
  ap_tibble %>% 
  select(fPosition, Age, Weight, Age:Weight, fPosition:Age, fPosition:Weight)
...
```{r}
summary(glmFit2 <- train(fSalary.Dummy ~ nTeam+nPosition+nCollege+poly(Age,3)+Height_i+Weight, data=ap_tibble, method="glmStepAIC"))
